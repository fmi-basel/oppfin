...
//TODO fix bug
        ...
        if(desc==null){
            ...
            desc = StringUtils.substringBetween(this.loadFromUrl(topicUrlDetail),
                tagOpen,tagEnd);
        }
        if (desc != null) {
            desc = desc.trim();
        } else {
            desc = "cannot get Description";
        }
        return desc;
    }
    ...
    public ItemProcessor<JSONObject, FieldMap> topicProcessor() {
        return new ItemProcessor<JSONObject, FieldMap>() {
            ...
            public FieldMap process(JSONObject topicObject) throws IOException {
                ...
                if(callList==null||callList.get(callFileName)==null) return null;
                //end check 
                if (!callList.get(callFileName).containsKey("callDescriptionHtml")) {
    ...
    public ItemProcessor<JSONObject, FieldMap> callProcessor() {
        return new ItemProcessor<JSONObject, FieldMap>() {
            ...
            public FieldMap process(JSONObject callObject) throws IOException {
                 ...
                 * "CallId":"H2020-MSCA-ITN-2014", "Status":"OPEN" }, "Title":
                 * "MARIE SKAODOWSKA-CURIE ACTION: INNOVATIVE TRAINING NETWORKS
                 * (ITN)", "FrameworkProgramme":"H2020",
                 ...
                 * "DeadlineDate":1397055600000, "CallDetails":{
                 * "AdditionalInfo":"", "LatestInfo":{
                 * "ApprovalDate":1391177582681, "Content": "The submission
                 * session is now available for:
                 * MSCA-ITN-2014-EID(MSCA-ITN-EID),
                 * MSCA-ITN-2014-EJD(MSCA-ITN-EJD),MSCA-ITN-2014-ETN(MSCA-ITN-ETN)"
                ...
                //TODO: add debug 
                String strDate = (String) callObject.get("PublicationDate");
                if (strDate.matches("\\d*")) {
                    strDate = df.format(new Date(Long.valueOf(strDate)));
                } else {
                    SimpleDateFormat dateFormat = new SimpleDateFormat("dd MMM yyyy");
                    try {
                        Date varDate = dateFormat.parse(strDate);
                        strDate = df.format(varDate);
                    } catch (Exception e) {
                        LOGGER.error("ParseException : " + e.getLocalizedMessage());
                    }
                }
//                LOGGER.error("==============>PublicationDate : " + strDate);
//                LOGGER.info(callObject.toJSONString());
                doc.put("callPublication", strDate);
                ...
                if (callObject.get("TotalIndicativeBudget") != null) {
                    try {
